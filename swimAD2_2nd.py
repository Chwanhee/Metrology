...
def you_shall_respect_me(hdwf, master_state, positive_state, negative_state, positive_voltage, negative_voltage):
    """
    ...
    dwf.FDwfAnalogIOChannelNodeSet(hdwf, c.c_int(0), c.c_int(1), c.c_double(positive_voltage))
 
    # set negative voltage
    negative_voltage = max(-5, min(0, negative_voltage))
    dwf.FDwfAnalogIOChannelNodeSet(hdwf, c.c_int(1), c.c_int(1), c.c_double(negative_voltage))
 
    # enable/disable the positive supply
    dwf.FDwfAnalogIOChannelNodeSet(hdwf, c.c_int(0), c.c_int(0), c.c_int(positive_state))
 
    # enable the negative supply
    dwf.FDwfAnalogIOChannelNodeSet(hdwf, c.c_int(1), c.c_int(0), c.c_int(negative_state))
 
    # start/stop the supplies - master switch
    dwf.FDwfAnalogIOEnableSet(hdwf, c.c_int(master_state))
    return
